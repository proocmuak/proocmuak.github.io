<template>
  <div class="homework-container">
    ...
    <div v-if="homework.length > 0" class="homework-list">
      ...
      <div class="homework-grid">
        <div 
          ...
          :class="{ 
            'low-score': hw.completionPercentage < 50
          }"
        >
          ...
          <div class="homework-actions">
            ...
            <button 
              v-if="hw.completionPercentage < 60" 
    ...
    <div v-if="!loading && homework.length === 0" class="no-data">
      Нет выполненных домашних работ для отображения
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async loadHomework() {
      ...
      try {
        ...
        // Получаем статусы завершения домашних работ (только выполненные)
        ...
        // Если нет выполненных работ — сразу выходим
        if (!completedHomework || completedHomework.length === 0) {
          this.homework = [];
          return;
        }

        // Оставляем только те работы, которые есть в списке выполненных
        const completedIds = new Set(completedHomework.map(hw => hw.homework_id));
        const filteredHomeworkList = homeworkList.filter(hw => completedIds.has(hw.homework_id));

        // Для каждой выполненной работы получаем максимальный балл из заданий
        const homeworkWithMaxScores = await Promise.all(
          filteredHomeworkList.map(async (homeworkItem) => {
        ...
        // Создаем полный список выполненных домашних работ
        ...
        // Сортируем по номеру домашней работы (по возрастанию)
        this.homework.sort((a, b) => a.homework_id - b.homework_id);
